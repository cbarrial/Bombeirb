...
void display_bomb(struct map* map, int x, int y, unsigned char type)
{
	...
	case BOMB_5:
			window_display_image(sprite_get_bomb(BOMB_5), x, y);
			break;
	}
}
...
struct map* map_get_map(int num)
{
	char chemin[11];
	snprintf(chemin,11,"data/map_%d",num);
	...
	fscanf(mapm,"%d:%d",&dim[0],&dim[1]);

	struct map*map=map_new(dim[0],dim[1]);

	fseek(mapm,3,SEEK_SET);
	int i,j;
	for (i=0;i<map->height;i++)
	{
		...
		{
			fscanf(mapm,"%d",&cell);
			map_set_cell_type(map,j,i,cell);
		}
